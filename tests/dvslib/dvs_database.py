...
class DVSDatabase:
    ...
    def wait_for_fields(
        ...
        if not status:
            assert not polling_config.strict, \
                "Expected fields not found: expected={}, \
                received={}, key=\"{}\", table=\"{}\"".format(expected_fields, result, key, table_name)
    ...
    def wait_for_field_match(
        ...
        if not status:
            assert not polling_config.strict, \
                "Expected field/value pairs not found: expected={}, \
                received={}, key=\"{}\", table=\"{}\"".format(expected_fields, result, key, table_name)
    ...
    def wait_for_field_negative_match(
        ...
        if not status:
            assert not polling_config.strict, \
                "Did not expect field/values to match, but they did: provided={}, \
                received={}, key=\"{}\", table=\"{}\"".format(old_fields, result, key, table_name)
    ...
    def wait_for_deleted_entry(
        ...
        if not status:
            assert not polling_config.strict, \
                "Entry still exists: entry={}, key=\"{}\", \
                    table=\"{}\"".format(result, key, table_name)
    ...
    def wait_for_n_keys(
        ...
        if not status:
            assert not polling_config.strict, \
                ...
                received={} ({}), table=\"{}\"".format(num_keys, len(result), result, table_name)
    ...
    def wait_for_matching_keys(
        ...
        if not status:
            assert not polling_config.strict, \
                "Expected keys not found: expected={}, received={}, \
                table=\"{}\"".format(expected_keys, result, table_name)
    ...
    def wait_for_deleted_keys(
        ...
        if not status:
            ...
            assert not polling_config.strict, \
                "Unexpected keys found: expected={}, received={}, \
                table=\"{}\"".format(expected, result, table_name)
