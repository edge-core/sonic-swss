...
bool PortsOrch::setDistributionOnLagMember(Port &lagMember, bool enableDistribution)
{
    ...
    status = sai_lag_api->set_lag_member_attribute(lagMember.m_lag_member_id, &attr);
    SWSS_LOG_INFO("first time %s distribution on LAG member %s return %d", enableDistribution ? "enable" : "disable", status, lagMember.m_alias.c_str());
    status = sai_lag_api->set_lag_member_attribute(lagMember.m_lag_member_id, &attr);
    SWSS_LOG_INFO("second time %s distribution on LAG member %s return %d", enableDistribution ? "enable" : "disable", status, lagMember.m_alias.c_str());
    if (status != SAI_STATUS_SUCCESS)
    {
